...
import { getDaysInMonth, formatDate, isScheduledForDay, isPast, isToday, getCurrentLogicalDate } from '@/lib/habitUtils';
...
export function HabitGrid({ tasks, completionMap, onToggle, onEditTask, onDeleteTask }: HabitGridProps) {
  ...
  const filteredTasks = tasks.filter((t) => {
    ...
    const timeA = useUIStore.getState().getTaskTime(a.id);
    const timeB = useUIStore.getState().getTaskTime(b.id);
          ...
          return (
            ...
            >
              ...
              {days.map((day) => {
                ...
                const isCarried = useUIStore.getState().getCarriedTasks(date).includes(task.id);
                const completed = !!completionMap[date]?.[task.id];
                const scheduled = isScheduledForDay(task, selectedYear, selectedMonth, day) || isCarried;
                    ...
                    {scheduled ? (
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="text-xs">
                          {missed ? `Missed — ${task.name}` : isCarried ? `[Carried] ${task.name} — Day ${day}` : `${task.name} — Day ${day}`}
