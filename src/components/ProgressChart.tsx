...
export function ProgressChart({ tasks, completionMap }: ProgressChartProps) {
  const { selectedMonth, selectedYear } = useUIStore();
  const data = getDailyCompletionData(tasks, completionMap, selectedMonth, selectedYear);
      ...
      <div className="h-[180px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            />
            <Line type="monotone" dataKey="rate" stroke="hsl(var(--primary))" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: 'hsl(var(--primary))' }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
